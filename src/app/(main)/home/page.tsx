...
export default function Home() {
    ...
    const ctx = gsap.context(() => {
      ...
      sections.forEach((sec, i) => {
        ...
        if (prevSec.cubePosition !== sec.cubePosition) {
          ...
          tl.to(glassRef.current, { left: targetLeft, duration: 1, ease: "power1.inOut" }, label)
            // First Half: Shrink and speed up as it reaches the center (duration 0.5)
            .to(cubeState, { scale: 0.3, duration: 0.5, ease: "sine.inOut" }, label)
            .to(cubeState, { speedMultiplier: 10, duration: 0.5, ease: "sine.inOut" }, label)
            // Second Half: Grow and slow down as it moves from center to the remaining edge
            .to(cubeState, { scale: 1, duration: 0.5, ease: "sine.inOut" }, `${label}+=0.5`)
            .to(cubeState, { speedMultiplier: 1, duration: 0.5, ease: "sine.inOut" }, `${label}+=0.5`);
        } else {
      ...
      const textSections = gsap.utils.toArray<HTMLElement>(".animate-text-section");
      ...
      const splits = textSections.map((section) => {
        ...
        tlText
          .fromTo(split.chars, 
            ...
            { opacity: 1, y: 0, scale: 1, stagger: 0.03, duration: 0.4, ease: "elastic.out(1, 0.5)" }
          )
          .to({}, { duration: 0.2 }) // Hold visible at center
          .to(split.chars, 
            { opacity: 0, y: -40, scale: 0.9, stagger: 0.02, duration: 0.4, ease: "power3.inOut" }
          );
  ...
  return (
    <div ref={containerRef} className="w-full h-max flex flex-col items-center justify-center">
      ...
      {sections.map((sec) => (
        <div key={sec.id} className="w-full flex min-h-svh py-16 items-center justify-center">
          {sec.cubePosition === "left" && <div className="w-1/2 h-full border max-lg:hidden border-transparent"></div>}
          <sec.Content />
          {sec.cubePosition === "right" && <div className="w-1/2 h-full border max-lg:hidden border-transparent"></div>}
        </div>
      ))}
